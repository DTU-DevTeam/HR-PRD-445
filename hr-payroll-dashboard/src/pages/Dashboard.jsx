...
const Dashboard = () => {
  ...
  useEffect(() => {
  fetch('https://localhost:7151/dashboard/dashboarddata')
    .then((res) => res.json())
    .then((data) => {
      //setGenderCount(data.countgender);
      setEmployeeCount(data.countemployees);
      setAverageSalary(data.averagesalary);
      //setDeptCount(data.countempindepartment);
      setIsLoading(false);
    })
    .catch((err) => {
